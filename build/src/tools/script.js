...
`),
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            const fn = await withTimeout(pageOrFrame.evaluateHandle(`(${request.params.function})`), DEFAULT_SCRIPT_TIMEOUT, 'Script evaluation timed out');
            args.unshift(fn);
            await context.waitForEventsAfterAction(async () => {
                const result = await withTimeout(pageOrFrame.evaluate(async (fn, ...args) => {
                    // @ts-expect-error no types.
                    return JSON.stringify(await fn(...args));
                }, ...args), DEFAULT_SCRIPT_TIMEOUT, 'Script execution timed out');
