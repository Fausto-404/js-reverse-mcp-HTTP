...
export const findInScript = defineTool({
    ...
    schema: {
        ...
        contextChars: zod
            ...
            .optional()
            .default(100)
            .describe('Number of characters to show before and after the match (default: 100).'),
        occurrence: zod
            .number()
            .int()
            .optional()
    ...
    handler: async (request, response, context) => {
        ...
        const { scriptId, query, contextChars, occurrence, caseSensitive } = request.params;
        try {
            const source = await debugger_.getScriptSource(scriptId);
            if (!source) {
                response.appendResponseLine(`No source found for script ${scriptId}.`);
            ...
            if (position === -1) {
                response.appendResponseLine(`"${query}" not found in script ${scriptId}${occurrence > 1 ? ` (occurrence ${occurrence})` : ''}.`);
                return;
            }
            ...
            const contextEnd = Math.min(source.length, position + query.length + contextChars);
            ...
            const afterContext = source.substring(position + query.length, contextEnd);
            ...
            response.appendResponseLine('```javascript');
            response.appendResponseLine(`${prefix}${beforeContext}【${matchText}】${afterContext}${suffix}`);
            response.appendResponseLine('```');
            response.appendResponseLine('');
            response.appendResponseLine(`To set a breakpoint here: set_breakpoint(url: "${url}", lineNumber: ${lineNumber + 1}, columnNumber: ${columnNumber})`);
        }
        catch (error) {
...
export const searchInSources = defineTool({
    ...
    schema: {
        ...
        maxLineLength: zod
            ...
            .optional()
            .default(150)
            .describe('Maximum characters per line preview (default: 150). Set to 0 for full lines.'),
        excludeMinified: zod
            .boolean()
            .optional()
            .default(true)
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            response.appendResponseLine('---');
            response.appendResponseLine('Tip: Use get_script_source(scriptId, startLine, endLine) to view full context around a match.');
        }
        catch (error) {
...
export const setBreakpoint = defineTool({
    ...
    schema: {
        url: zod
            .string()
            .describe('The URL of the JavaScript file (can be a partial match or regex pattern).'),
        lineNumber: zod
            .number()
            .int()
        ...
        condition: zod
            .string()
            .optional()
            .describe('Optional condition expression. The breakpoint only triggers when this evaluates to true.'),
        isRegex: zod
            .boolean()
            .optional()
            .default(false)
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            if (isRegex) {
                breakpointInfo = await debugger_.setBreakpointByUrlRegex(url, line0based, columnNumber, condition);
            }
            else {
                breakpointInfo = await debugger_.setBreakpoint(url, line0based, columnNumber, condition);
            }
            ...
            response.appendResponseLine(`Error setting breakpoint: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
});
/**
...
export const evaluateOnCallframe = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        if (frameIndex >= pausedState.callFrames.length) {
            response.appendResponseLine(`Invalid frame index ${frameIndex}. Available frames: 0-${pausedState.callFrames.length - 1}`);
            return;
        }
        ...
        try {
            ...
            response.appendResponseLine(`Error evaluating: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
});
/**
...
export const hookFunction = defineTool({
    ...
    schema: {
        target: zod
            .string()
            .describe('The function to hook. Can be: global function name ("fetch"), object method ("XMLHttpRequest.prototype.open"), or path ("window.app.api.request").'),
        logArgs: zod
            .boolean()
            .optional()
            .default(true)
        ...
        hookId: zod
            .string()
            .optional()
            .describe('Custom identifier for this hook. Used to unhook later. Defaults to target name.'),
    },
    handler: async (request, response, context) => {
        ...
        try {
            ...
            if (result && typeof result === 'object') {
                if (result.success) {
                    ...
                    response.appendResponseLine('');
                    response.appendResponseLine('Function calls will be logged to console. Use list_console_messages to view.');
...
`;
        try {
            const page = context.getSelectedPage();
            const hooks = (await page.evaluate(listCode));
...
export const inspectObject = defineTool({
    ...
    schema: {
        expression: zod
            .string()
            .describe('JavaScript expression to evaluate and inspect (e.g., "window.app", "document.body", "myObject").'),
        depth: zod
            .number()
            .int()
            .optional()
...
export const monitorEvents = defineTool({
    ...
    schema: {
        selector: zod
            ...
            .optional()
            .default('window')
            .describe('CSS selector for element to monitor, or "window"/"document" (default: window).'),
        events: zod
            .array(zod.string())
            .optional()
            .describe('Specific events to monitor (e.g., ["click", "keydown"]). If not specified, monitors common events.'),
        monitorId: zod
            .string()
            .optional()
        ...
        try {
            ...
            if (result && typeof result === 'object') {
                if (result.success) {
                    ...
                    response.appendResponseLine('');
                    response.appendResponseLine('Events will be logged to console. Use list_console_messages to view.');
                    response.appendResponseLine(`Use stop_monitor(monitorId: "${r.monitorId}") to stop.`);
                }
                else {
                    response.appendResponseLine(`❌ ${result.message}`);
                }
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            if (!foundMatch) {
                ...
                response.appendResponseLine('');
                response.appendResponseLine('Tip: Use search_in_sources to find the exact function signature, then use set_breakpoint.');
                return;
            }
