...
```bash
node build/src/http-bridge.js --port 8787

# 需要传递参数给 stdio MCP 时，用 -- 追加
node build/src/http-bridge.js --port 8787 -- --browser-url http://127.0.0.1:9222
```

在 Dify 的 “添加 MCP 服务 (HTTP)” 中填写：

- 服务端点 URL：`http://127.0.0.1:8787/mcp`
- 名称：`js-reverse`
- 服务器标识符：`js-reverse`
- 认证：无需（留空）

如果 Dify 运行在 Docker 内，请把地址替换为宿主机可访问的地址（如 `host.docker.internal`）并确认端口映射。
